...
In the function `setPortForwardRules`,  attackers can post the parameter `comment` without checking its' length, which can lead to buffer overflows in the following `strcpy` function.
...
Connection: close
Referer: http://192.168.0.254/firewall/port_forward.asp
